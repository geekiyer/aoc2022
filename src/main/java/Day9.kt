...
fun main() {
    val input = object {}.javaClass.getResourceAsStream("day9.txt")?.bufferedReader()?.readLines()!!
    println(solve1(input))
    println(solve2(input))
}

private fun solve1(lines: List<String>): String {
    val steps = parseSteps(lines)

    var positionHead = Vector2D(0, 0)
    var positionTail = positionHead
    val positionsTail = mutableListOf<Vector2D>()
...
private fun solve2(lines: List<String>): String {
    val steps = parseSteps(lines)
